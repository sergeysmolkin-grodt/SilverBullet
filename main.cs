    ...
    namespace cAlgo.Robots
    {
        ...
        public class SilverBulletBot : Robot
        {
            ...
            [Parameter("Contract Multiplier", DefaultValue = 1.0, MinValue = 0.01, Group = "Strategy Parameters")]
            ...
            [Parameter("ZigZag Depth", DefaultValue = 12, MinValue = 2, Group = "ZigZag")]
            public int ZigZagDepth { get; set; }
            [Parameter("ZigZag Deviation", DefaultValue = 5, MinValue = 1, Group = "ZigZag")]
            public int ZigZagDeviation { get; set; }
            [Parameter("ZigZag BackStep", DefaultValue = 3, MinValue = 1, Group = "ZigZag")]
            ...
            protected override void OnStart()
            {
                ...
                _zigZag = Indicators.GetIndicator<ZigZag>(
                    ZigZag.ModeZigZag.HighLow,
                    ZigZagDepth,
                    ZigZagDeviation,
                    ZigZagBackStep,
                    false, // ShowLabel
            ...
            protected override void OnTick()
            {
                ...
                // --- ZigZag BOS Entry Logic ---
                if (_lastSweepType != SweepType.None && _zigZag != null)
                {
                    var bars = MarketData.GetBars(_executionTimeFrame);
                    int lastBarIndex = bars.Count - 2; // Последний закрытый бар
                    if (lastBarIndex < 1) return;

                    // Найти последний экстремум ZigZag
                    int zzIndex = -1;
                    double zzLevel = 0;
                    bool isLow = false, isHigh = false;
                    for (int i = lastBarIndex; i >= Math.Max(0, lastBarIndex - 10); i--)
                    {
                        if (_zigZag.Result[i] > 0)
                        {
                            zzIndex = i;
                            zzLevel = _zigZag.Result[i];
                            isHigh = bars.HighPrices[i] == zzLevel;
                            isLow = bars.LowPrices[i] == zzLevel;
                            break;
                        }
                    }
                    if (zzIndex > 0 && zzLevel != 0)
                    {
                        // Проверяем BOS по ZigZag
                        var close = bars.ClosePrices[lastBarIndex];
                        var open = bars.OpenPrices[lastBarIndex];
                        if (_lastSweepType == SweepType.LowSwept && isLow && close > zzLevel && open < zzLevel)
                        {
                            // Bullish BOS по ZigZag: вход в Buy
                            _isProcessingSetup = true;
                            Print($"ZigZag BOS: Bullish. Вход Buy по рынку. ZigZag Low={zzLevel} на баре {bars.OpenTimes[zzIndex]}");
                            PrepareAndPlaceZigZagBOSEntry(TradeType.Buy, zzLevel, lastBarIndex);
                            return;
                        }
                        if (_lastSweepType == SweepType.HighSwept && isHigh && close < zzLevel && open > zzLevel)
                        {
                            // Bearish BOS по ZigZag: вход в Sell
                            _isProcessingSetup = true;
                            Print($"ZigZag BOS: Bearish. Вход Sell по рынку. ZigZag High={zzLevel} на баре {bars.OpenTimes[zzIndex]}");
                            PrepareAndPlaceZigZagBOSEntry(TradeType.Sell, zzLevel, lastBarIndex);
                            return;
                        }
                    }
                }
            ...
            private double CalculateOrderVolume(double stopLossInPoints)
            {
                Print("--- Расчет объема ордера ---");
                Print($"Входной StopLoss в пунктах (points): {stopLossInPoints}");
                Print($"Используемый множитель контракта: {ContractMultiplier}");

                if (stopLossInPoints <= 0 || ContractMultiplier <= 0)
                {
                    Print("Неверное значение стоп-лосса или множителя. Расчет объема прерван.");
                    return 0;
                }

                double accountBalance = Account.Balance;
                double riskAmount = accountBalance * (RiskPercentage / 100.0);
                Print($"Баланс счета: {accountBalance}, Процент риска: {RiskPercentage}%, Сумма риска: {riskAmount}");

                // Стоимость риска для 1 лота = дистанция в пунктах * стоимость 1 пункта для 1 лота (множитель)
                double riskValuePerLot = stopLossInPoints * ContractMultiplier;

                if (riskValuePerLot <= 0)
                {
                    Print($"Ошибка: Рассчитанная стоимость риска на лот равна {riskValuePerLot}. Расчет объема невозможен.");
                    return 0;
                }
                
                double volume = riskAmount / riskValuePerLot;

                Print($"Первоначальный объем (в лотах, ручной расчет): {volume}");
                ...
                Print($"Ограничения символа: MinОбъем={minVolume}, MaxОбъем={maxVolume}, Шаг={volumeStep}");

                volume = Math.Max(minVolume, Math.Min(maxVolume, volume));
                volume = Symbol.NormalizeVolumeInUnits(volume, RoundingMode.Down);
                
                Print($"--- Итоговый рассчитанный объем (Лоты): {volume} ---");
                return volume;
            }
            ...
            private void PrepareAndPlaceFVGEntryOrder(SweepType bosDirection)
            {
                ...
                // Проверяем, не слишком ли далеко цена ушла от уровня входа
                double currentPrice = (tradeType == TradeType.Buy) ? Symbol.Ask : Symbol.Bid;
                double priceDistance = Math.Abs(currentPrice - entryPrice);
                double maxAllowedDistance = Symbol.PipSize * 5; // Максимальное расстояние 5 пипов

                if (priceDistance > maxAllowedDistance)
                {
                    Print($"FVG entry price {entryPrice} is too far from current price {currentPrice} (distance: {priceDistance} pips). Order not placed.");
                    ResetSweepAndBosState("FVG entry price too far from market.");
                    return;
                }
                ...
                double volume = CalculateOrderVolume(Math.Abs(entryPrice - stopLossPrice));
                if (volume == 0)
                {
                ...
                // Если цена уже близко к уровню входа, используем рыночный ордер
                if ((tradeType == TradeType.Buy && entryPrice >= Symbol.Ask) || (tradeType == TradeType.Sell && entryPrice <= Symbol.Bid))
                {
                    Print($"Price already at/through FVG level. Using market order instead of limit order.");
                    newOrderLabel = $"SB_FVG_MKT_{SymbolName}_{Server.Time:yyyyMMddHHmmss}";
                    ...
                    ExecuteMarketOrderAsync(tradeType, SymbolName, volume, newOrderLabel, null, null, tradeResult =>
                    {
                        if (tradeResult.IsSuccessful)
                        {
                            ...
                            Print($"Successfully placed market order (FVG) {newOrderLabel}. Position: {position.Id}");

                            var slResult = position.ModifyStopLossPrice(stopLossPrice);
                            if (slResult.IsSuccessful) Print($"Successfully set SL for FVG market order {position.Id} to {stopLossPrice}.");
                            else Print($"Failed to set SL for FVG market order {position.Id}: {slResult.Error}");

                            var tpResult = position.ModifyTakeProfitPrice(takeProfitPrice);
                            if (tpResult.IsSuccessful) Print($"Successfully set TP for FVG market order {position.Id} to {takeProfitPrice}.");
                            else Print($"Failed to set TP for FVG market order {position.Id}: {tpResult.Error}");

                            // OnPositionOpened will handle this, but we can do it here to be safe as well.
                            // _tradesTakenToday++; 
                            ResetSweepAndBosState($"Market order {newOrderLabel} processed, resetting state.");
                        }
                        else
                        {
                            Print($"Failed to place market order (FVG) {newOrderLabel}: {tradeResult.Error}.");
                            ResetSweepAndBosState($"Failed to place FVG market order: {tradeResult.Error}");
                        }
                    });
                    return;
                }
            ...
            private void PrepareAndPlaceBOSEntryOrder(SweepType bosDirection)
            {
                ...
                double stopLossInPoints = Math.Abs(entryPrice - stopLossPrice);
                if (stopLossInPoints <= 0.5)
                {
                    Print($"Stop loss is too small ({stopLossInPoints} points for BOS market entry). Min SL: 0.5 points. Order not placed.");
                ...
                double volume = CalculateOrderVolume(stopLossInPoints);
                if (volume == 0)
                {
            ...
            private double FindTakeProfitLevel(TradeType tradeType, double entryPrice, double stopLossPrice)
            {
                ...
                {
                    Print($"Stop loss distance ({stopLossDistance}) is zero or negative. Aborting TP calculation.");
                    return 0;
                }
                ...
                {
                    Print($"Searching for TP among {_tpTargetLiquidityLevels.Count} potential liquidity targets. SL pips: {stopLossDistance:F2}. Entry: {entryPrice}.");
                    ...
                    foreach (var targetLevel in _tpTargetLiquidityLevels)
                    {
                        ...
                        if (tradeType == TradeType.Buy)
                        {
                            ...
                            {
                                double targetDistance = (potentialTarget - entryPrice);
                                if (targetDistance <= 0) continue;

                                double currentRR = targetDistance / stopLossDistance;
                        ...
                        else // Sell Trade
                        {
                            if (potentialTarget < entryPrice)
                            {
                                double targetDistance = (entryPrice - potentialTarget);
                                if (targetDistance <= 0) continue;
            ...
            // Новый метод для входа по ZigZag BOS
            private void PrepareAndPlaceZigZagBOSEntry(TradeType tradeType, double zzLevel, int barIndex)
            {
                var bars = MarketData.GetBars(_executionTimeFrame);
                double entryPrice = (tradeType == TradeType.Buy) ? Symbol.Ask : Symbol.Bid;
                double stopLossPrice = zzLevel;
                double stopLossInPoints = Math.Abs(entryPrice - stopLossPrice);
                if (stopLossInPoints < 0.5)
                {
                    Print($"ZigZag BOS: SL слишком мал ({stopLossInPoints} points). Вход отменён.");
                    ResetSweepAndBosState("ZigZag BOS: SL слишком мал.");
                    return;
                }
                double takeProfitPrice = FindTakeProfitLevel(tradeType, entryPrice, stopLossPrice);
                if (takeProfitPrice == 0)
                {
                    Print($"ZigZag BOS: Не найден TP с нужным RR. Вход отменён.");
                    ResetSweepAndBosState("ZigZag BOS: Нет TP.");
                    return;
                }
                double volume = CalculateOrderVolume(stopLossInPoints);
                if (volume == 0)
                {
                    Print("ZigZag BOS: Volume=0. Вход отменён.");
                    ResetSweepAndBosState("ZigZag BOS: Volume=0.");
                    return;
                }
                string label = $"SB_ZZBOS_{SymbolName}_{Server.Time:yyyyMMddHHmmss}";
                Print($"ZigZag BOS: Открываю {tradeType} по рынку. SL={stopLossPrice}, TP={takeProfitPrice}, Vol={volume}");
                ExecuteMarketOrderAsync(tradeType, SymbolName, volume, label, null, null, tr =>
                {
                    if (tr.IsSuccessful)
                    {
                        var pos = tr.Position;
                        var slRes = pos.ModifyStopLossPrice(stopLossPrice);
                        var tpRes = pos.ModifyTakeProfitPrice(takeProfitPrice);
                        Print($"ZigZag BOS: Позиция {pos.Id} открыта. SL: {slRes.IsSuccessful}, TP: {tpRes.IsSuccessful}");
                    }
                    else
                    {
                        Print($"ZigZag BOS: Ошибка открытия позиции: {tr.Error}");
                    }
                    ResetSweepAndBosState("ZigZag BOS: Вход обработан.");
                });
            }
        }
    }
